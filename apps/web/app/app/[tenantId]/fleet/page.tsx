  ...
  const installedWorkflows = useMemo(() => {
    return rawInstalledWorkflows.map((installation) => {
      const workflow = workflowTemplates.find(
        (template) => template.id === installation.templateId
      );
      return {
        ...installation,
        workflow,
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <SpotlightCard className="rounded-md border border-border/50 bg-card">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">of {installedAgents.length} installed</p>
          </CardContent>
        </SpotlightCard>
        <SpotlightCard className="rounded-md border border-border/50 bg-card">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <SpotlightCard className="rounded-md border border-border/50 bg-card">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">across all agents and workflows</p>
          </CardContent>
        </SpotlightCard>
        <SpotlightCard className="rounded-md border border-border/50 bg-card">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <AlertDialogFooter>
            ...
            >
              {isLoading ? (
                ...
                'Pause'
              ) : actionState?.type === 'resume' ? (
                'Resume'
              ) : actionState?.type === 'trigger' ? (
                'Trigger'
              ) : (
                'Duplicate'
              )}
