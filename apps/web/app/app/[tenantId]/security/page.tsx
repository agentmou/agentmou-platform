...
import { Shield, Key, Users, Plus, CheckCircle2, Lock, Download, FileQuestion } from 'lucide-react';
...
export default function SecurityPage() {
  ...
  const filteredAuditEvents =
    auditFilter === 'all' ? auditEvents : auditEvents.filter((e) => e.category === auditFilter);
    ...
    <div className="p-6 lg:p-8 space-y-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Manage secrets, access controls, and audit logs
        </p>
      </div>
              ...
              {findings.slice(0, 3).map((finding) => (
                <div key={finding.id} className="rounded-lg border border-border/50 p-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <StatusPill status={findingStatus(finding.severity)} label={finding.severity} />
                  </div>
          ...
          <Card className="border-border/50">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription className="text-xs">
                  API keys, tokens, and credentials used by your agents
                </CardDescription>
              </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-b border-border/50 hover:bg-transparent">
                    <TableHead className="h-11 text-[11px] uppercase tracking-[0.05em] text-muted-foreground font-medium bg-muted/30">
                      Key
                    </TableHead>
                    <TableHead className="h-11 text-[11px] uppercase tracking-[0.05em] text-muted-foreground font-medium bg-muted/30">
                      Value
                    ...
                    <TableHead className="h-11 text-[11px] uppercase tracking-[0.05em] text-muted-foreground font-medium bg-muted/30 text-right">
                      Status
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {serverSecrets.length === 0 ? (
                    ...
                    serverSecrets.map((secret) => (
                      <TableRow
                        key={secret.id}
                        className="border-b border-border/30 h-[52px] hover:bg-muted/20"
                      >
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-b border-border/50 hover:bg-transparent">
                      ...
                      <TableHead className="h-11 text-[11px] uppercase tracking-[0.05em] text-muted-foreground font-medium bg-muted/30 text-center">
                        Viewer
                      </TableHead>
                      <TableHead className="h-11 text-[11px] uppercase tracking-[0.05em] text-muted-foreground font-medium bg-muted/30 text-center">
                        Editor
                  ...
                  <TableBody>
                    {[
                      ...
                      <TableRow
                        key={perm.name}
                        className="border-b border-border/30 h-[44px] hover:bg-transparent"
                      >
                        <TableCell className="text-sm">{perm.name}</TableCell>
                        <TableCell className="text-center">
                          {perm.viewer ? (
                            <CheckCircle2 className="mx-auto h-3.5 w-3.5 text-accent" />
                          ) : (
            ...
            <Card className="border-border/50">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription className="text-xs">
                    Complete history of actions in your workspace
                  </CardDescription>
                </div>
