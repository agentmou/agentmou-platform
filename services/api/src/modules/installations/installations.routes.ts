...
export async function installationRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    '/tenants/:tenantId/installations/:installationId',
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { tenantId, installationId } = request.params as {
        tenantId: string;
  ...
  fastify.post(
    '/tenants/:tenantId/installations/agents',
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { tenantId } = request.params as { tenantId: string };
      const { templateId, config } = request.body as {
        templateId: string;
  ...
  fastify.post(
    '/tenants/:tenantId/installations/workflows',
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { tenantId } = request.params as { tenantId: string };
      const { templateId, config } = request.body as {
        templateId: string;
  ...
  fastify.post(
    '/tenants/:tenantId/installations/packs',
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { tenantId } = request.params as { tenantId: string };
      const { packId, config } = request.body as {
        packId: string;
        config?: Record<string, unknown>;
      };
